...
Meta_all <- read_excel("./data/Validation/MASTER.xlsx",col_names=TRUE)
Meta_all[] <- lapply(Meta_all, factor)

RNA_all <- read.csv("./data/Validation/counts.csv", row.names=1)
...
corto.groups = gmtPathways("./data/db/canonical.gmt")
...
DPA_fit = lmFit(RNA_baseline,DPA_design)
...
DPA_top
write.csv(DPA_top,'./Reports/Validation/ART-TNBC_DGE_baseline_PEVresp2PEVres.csv')
...
hm_input = RNA_baseline[DPA_top_n_names,]

plot_hm = Heatmap(hm_input,
             name = "ssGSEA",
             top_annotation = anno_gen(Meta_baseline %>% dplyr::select(c("Class_"))),
             show_row_names = TRUE,
...
```{r}
library(stringr)
df = RNA_exp_cpm_filter[which(str_detect(row.names(RNA_exp_cpm_filter),"SUMO|NEDD")),Cohort]

plot_hm = Heatmap(df,
             name = "mRNA",
             top_annotation = anno_gen(Meta_baseline %>% dplyr::select(c("PDX_","Class_"))),
             show_row_names = TRUE,
             row_names_gp = gpar(fontsize = 10),
             row_names_max_width = max_text_width(rownames(hm_input),gp = gpar(fontsize = 10)),
             show_column_names = FALSE,
...
DPA_top
write.csv(DPA_top,'./Reports/Validation/ART-TNBC_ssGSEA_baseline_PEVresp2PEVres.csv')
...
QUESTION_2a: What changes as a function of time at a pathway level with PEV exposure
...
QUESTION_2a: What changes as a function of time at a gene level with PEV exposure
```{r fig.height=20, fig.width=24}
library(sigFeature)
library(dplyr)
thresh_Pval = 0.0001
...
SigFeat_Y_drug = Meta_all[Cohort_Drug,] %>% dplyr::select("Time_")
SigFeat_X_drug = t(RNA_exp_cpm_filter[,Cohort_Drug])
SigFeat_X_drug = SigFeat_X_drug[,order(apply(SigFeat_X_drug,2,sd),decreasing = T)]
SigFeat_X_drug = SigFeat_X_drug[,1:1000]
SigFeat_X_drug_names = colnames(SigFeat_X_drug)
sigFeat_drug = unlist(sigFeaturePvalue(x=SigFeat_X_drug,y=SigFeat_Y_drug))
...
SigFeat_Y_VEH = Meta_all[Cohort_VEH,] %>% dplyr::select("Time_")
SigFeat_X_VEH = t(RNA_exp_cpm_filter[SigFeat_X_drug_names,Cohort_VEH])
SigFeat_X_VEH_names = colnames(SigFeat_X_VEH)
sigFeat_VEH = unlist(sigFeaturePvalue(x=SigFeat_X_VEH,y=SigFeat_Y_VEH))
names(sigFeat_VEH) = SigFeat_X_VEH_names
sigGene_VEH_sig = sigFeat_VEH[which(sigFeat_VEH<0.05)]
...
df.top = df[1:25,]
df.top.names = row.names(df.top)
hm_input = RNA_exp_cpm_filter[df.top.names,]
#row.names(hm_input)[order(row.names(hm_input))]
...
HM = Heatmap(hm_input,
             ...
             show_row_names = TRUE,
             row_names_gp = gpar(fontsize = 15),
             row_names_max_width = max_text_width(rownames(hm_input),gp = gpar(fontsize = 15)),
             heatmap_legend_param = list(legend_direction = "horizontal", legend_width = unit(6, "cm"), title_position = "topcenter")
             )
draw(HM, heatmap_legend_side="bottom", annotation_legend_side="left",
           legend_grouping = "adjusted")
