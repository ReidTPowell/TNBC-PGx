...
rm("pfr_tmp")
```
Get data from in vitro correlation to specific targets
```{r}
t(rbind(Drug_all["PEVONEDISTAT_Broad",train_samples],RNA_all[c("NAE1","SAE1"),train_samples]))
```
Get data from in vivo correlation to specific targets
```{r}
cbind(as.matrix(Meta_all[train_samples,"InVivo_PEV"]),t(RNA_all[c("NAE1","SAE1"),train_samples]))
```
