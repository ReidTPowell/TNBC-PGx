...
```
```{r}
set1 = Drug_all["EPIRUBICINHYDROCHLORIDE_NCI_Onc_AOD",colnames(Drug_all) %in% train_samples]
set2 = Drug_all["EPIRUBICINHYDROCHLORIDE_NCI_Onc_AOD",!(colnames(Drug_all) %in% train_samples)]
```
...
```{r}
tmp[,c("InVivo_PEV","NAE1","SAE1")]
```

```{r}
sel = intersect(colnames(RNA_filtered_z),colnames(Drug_all))

cbind(t(Drug_all["PEVONEDISTAT_Broad",sel]),t(RNA_filtered_z[c("NAE1","SAE1"),sel]))
```
