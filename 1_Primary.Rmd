...
library(fgsea)
library(corto)
pathways = gmtPathways("./data/db/canonical.gmt")
...
Trains a series of L1 regularized (lasso) regression models. Each model is trained against a single gene set and the RMSE is used to rank the importance of individual pathways. Feature importance analysis provide a more grainular view of the individual genes that are driving the decision making.  
```{r}
library(stringr)
library(foreach)
library(doParallel)
...
design = model.matrix(~0+dAUC_outlier+Time+PiD,metadata)
DEG_fit = lmFit(RNA_filtered_z[,train_samples],design)
cm = makeContrasts(Mid_v_Pre=dAUC_outlierTRUE-dAUC_outlierFALSE,levels=design)
DEG_fit_cm = contrasts.fit(DEG_fit,cm)
DEG_fit_cm_eBayesfit = eBayes(DEG_fit_cm,trend = FALSE,robust = FALSE)
DEG_top = topTable(DEG_fit_cm_eBayesfit,adjust.method="fdr", number=Inf)
DEG_top
...
tmp = DEG_top[intersect(which(DEG_top$adj.P.Val <= thresh_Pval),which(abs(DEG_top$logFC) >= thresh_FC)),]
up = as.integer(length(which(tmp$logFC > 0)))
down =  as.integer(length(which(tmp$logFC <= 0)))
print(paste("Total DEG: ", up+down))
print(paste("Sigificantly up-reglated genes: ", up))
...
#Volcano plot of DEG
fig <- plot_ly(data = DEG_top, x = ~logFC, y = ~-1*log10(adj.P.Val),color = ~logFC, colors="RdBu")%>%
  layout(xaxis = list(title = "DeltaZ", tickfont = list(size = 40),titlefont = list(size = 60)),
         yaxis = list(title = "-log(p-value)", tickfont = list(size = 40),titlefont = list(size = 60)),
         shapes=list(vline(-1*thresh_FC),vline(thresh_FC),hline(-log10(thresh_Pval)))) %>%
  add_trace(text=row.names(DEG_top))
...
```{r}
library(stringr)
names(pathways)[which(str_detect(names(pathways),"MULTI"))]
```
```{r fig.height=12, fig.width=14}
library(ComplexHeatmap)

#gene_list = intersect(row.names(RNA_filtered_z),pathways[["KEGG_DRUG_METABOLISM_CYTOCHROME_P450"]])
gene_list = c("SAE1","SUMO1","NAE1","NEDD8","PTEN","AKT1","AKT2","AKT3")

#subset = train_samples
subset = row.names(Meta_all)[which(Meta_all$dAUC_outlier == "TRUE")]
df_anno = Meta_all[subset,]%>%dplyr::select("Time","PiD")

Heatmap(RNA_filtered_z[gene_list,subset],
        top_annotation = anno_gen(df_anno))
```
...
pfR_input = DEG_top[,c("logFC","adj.P.Val"),]
Gene_symbol = row.names(pfR_input)
pfR_input = cbind(Gene_symbol,pfR_input)

name = "Reports/Primary/DEG_LmPiD_Time_RespOutlier_pathfindR"
